...
const CallReport = () => {
    ...
    useEffect(() => {
        ...
        if (filter !== 'all') {
            filtered = filtered.filter(report => {
                ...
                // if (filter === 'potentialLead') return report.summary && report.summary.toLowerCase().includes('counse');
        ...
        setFilteredReports(filtered.slice(0, 10000));
    }, [searchQuery, filter, reports]);
    ...
    const getCount = (filter) => {
        ...
        // if (filter === 'potentialLead') return validReports.filter(report => report.summary && report.summary.toLowerCase().includes('counse')).length;
                ...
                {selectedReport && (
                    <DialogContent>
                        <DialogContentText>
                            ...
                            <strong>Ended At:</strong> {selectedReport.endedAt || 'N/A'}<br />
